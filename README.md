...
uv run python -m kev.benchmark --run jaredpalmer/kev-4b --suite evals/external/catsbench-v1 --out runs/kev-4b-catsbench-v1 --device cuda  # time-series observation + forecast
...
`catsbench-v1` converts CaTSBench captions and numeric series into observation and forecast questions. The command above writes per-question predictions to `runs/kev-4b-catsbench-v1/rows.json` and aggregate metrics to `report.json`. See the [suite card](evals/external/catsbench-v1/README.md) for the task families, construction rules, initial results, and pinned source revisions. The converted dataset is also published at [Aadharsh/cats-bench-kev](https://huggingface.co/datasets/Aadharsh/cats-bench-kev).
