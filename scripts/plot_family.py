...
import numpy as np
from chartstyle import GRID, JEV, KEV, RULE, TEXT, TEXT2, body, heading, rule, strip, use_style
...
MODELS = [("kev-0.6b", "runs/v7-06b/02-trial-2/result.json"), ("kev-4b", "runs/v7-rc3/01-trial-1/result.json"), ("kev-8b", "runs/v7-final/00-trial-0/result.json")]
...
def main():
    ...
    kx = .27
    for i, (n, c) in enumerate([("Jev", JEV), ("kev-8b", KEV["kev-8b"]), ("kev-4b", KEV["kev-4b"]), ("kev-0.6b", KEV["kev-0.6b"])]):
        fig.text(kx + .085 * i, .835, "●", fontsize=13, color=c, ha="left", va="baseline"); fig.text(kx + .085 * i + .014, .835, n, fontsize=11.5, color=TEXT, ha="left", va="baseline")
    ...
    body(fig, .05, .018, "LoRA r=16 + pointer head; all three on decision-v7; kev-0.6b at lr 1e-4, kev-4b and kev-8b at lr 5e-5.  Regenerate: uv run python scripts/plot_family.py", size=10)
