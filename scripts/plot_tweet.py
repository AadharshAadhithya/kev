...
from chartstyle import JEV, KEV, NEUTRAL, TEXT, TEXT2, body, heading, hbars, rule, stat, use_style
...
def main():
    ...
    ax = fig.add_axes([.21, .235, .42, .52])
    heading(fig, .05, .77, "Accuracy on data kev never trained on", size=17)
    hbars(ax, [r[0] for r in rows], [r[2] for r in rows], [r[3] for r in rows], xlim=(0, 100), fmt="{:.1f}%",
    ...
    stat(fig, x, .37, "Trained parameters, kev-4b and kev-8b", "34M · 46M", "LoRA r=16 + pointer head on a frozen base\n40–70 min on one H100; serves on a 32 GB Mac", color=TEXT, value_size=30)
